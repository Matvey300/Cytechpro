...
def week_agg_reviews(df_reviews: pd.DataFrame) -> pd.DataFrame:
    ...
    for star, colname in [(5, 'p5_share_week'), (1, 'p1_share_week')]:
        tmp = df_reviews.assign(is_star=(df_reviews['rating'] == star).astype(int)) \
...
def detect_sales_column(df: pd.DataFrame) -> Optional[str]:
    ...
    return find_column(df, [
        'weekly_sales', 'units_sold', 'units_week', 'sales_units', 'sales',
        'sold', 'qty', 'quantity', 'units'
    ])

def detect_price_column(df: pd.DataFrame) -> Optional[str]:
    """Detect a likely price column name."""
    return find_column(df, [
        'price', 'avg_price', 'bb_new_landed_price', 'bb_new_landed_price_avg_3mo',
        'bb_new_landed_price_avg_1mo', 'price_avg', 'buy_box_price'
    ])
...
def build_master_weekly(df_reviews: Optional[pd.DataFrame],
                        df_sync: Optional[pd.DataFrame]) -> Tuple[pd.DataFrame, Dict]:
    ...
    if df_reviews is not None and not df_reviews.empty:
        ...
        if date_col:
            ...
            if 'asin' in df.columns and 'rating' in df.columns:
                ...
                weekly_reviews['bayes_rating_week'] = weekly_reviews.apply(bayesian_adjusted_rating, axis=1)
    ...
    for asin, g in df.groupby('asin'):
        ...
        if len(g) < 3:
            feats.append({'asin': asin, 'burstiness': np.nan, 'recent_shift': np.nan,
                          'extremeness': np.nan, 'drift_vs_cum': np.nan, 'obs_weeks': len(g)})
            continue
        ...
        recent_shift = (last4['avg_rating_week'].mean() - prior['avg_rating_week'].mean()) if len(prior) > 0 else np.nan
...
def compute_review_sales_impact(master: pd.DataFrame,
                                min_weeks: int = 8) -> Dict[str, pd.DataFrame]:
    """
    ...
    metrics = ['avg_rating_week', 'reviews_count_week', 'p5_share_week',
               'bayes_rating_week', 'price_change_weekly', 'avg_price_week']
    per_rows = []
    for asin, g in master.groupby('asin'):
        ...
        for m in metrics:
            if m in g.columns and g[m].notna().sum() >= min_weeks:
                ...
                if aligned.shape[0] >= min_weeks:
                    r = np.corrcoef(aligned['weekly_sales'].astype(float),
                                    aligned[m].astype(float))[0, 1]
                    per_rows.append({'asin': asin, 'metric': m, 'type': 'contemporaneous', 'corr': r})
        ...
        for m in metrics:
            if m in g.columns:
                ...
                if aligned.shape[0] >= min_weeks:
                    r = np.corrcoef(aligned['weekly_sales'].astype(float),
                                    aligned[f'{m}_lag1'].astype(float))[0, 1]
                    per_rows.append({'asin': asin, 'metric': f'{m}_lag1', 'type': 'lag1', 'corr': r})
    ...
    if not pooled.empty:
        ...
        for m in metrics:
            ...
            if col in pooled.columns:
                ...
                if aligned.shape[0] >= min_weeks:
                    r = np.corrcoef(aligned['sales_dm'].astype(float), aligned[col].astype(float))[0, 1]
...
def build_volatility(master: pd.DataFrame) -> pd.DataFrame:
    ...
    return master.groupby('asin').agg(
        weeks=('week', 'nunique'),
...
# ----------------------------- Main CLI -----------------------------
def main():
    parser = argparse.ArgumentParser(description="Amazon Reviews & Sales – Weekly Analytics Pipeline (comments in English only).")
    parser.add_argument('--reviews', type=str, required=True, help='Path to reviews CSV (e.g., all_reviews.csv).')
    parser.add_argument('--synccentric', type=str, required=False, default=None, help='Path to sales/price CSV (e.g., output total synccentric.csv).')
    parser.add_argument('--outdir', type=str, required=True, help='Output directory to write results.')
    parser.add_argument('--do-cluster', type=int, default=0, help='If >0, run simple KMeans with k clusters.')
    ...
    # Build master weekly
    master, info = build_master_weekly(df_reviews if df_reviews is not None else pd.DataFrame(),
    ...
    if not weekly_reviews_only.empty:
        dist = compute_distortion_score(
            weekly_reviews_only[['asin','week','avg_rating_week','reviews_count_week',
                                 'p5_share_week','rating_var_week','cum_avg_rating']].dropna(how='all')
        )
    else:
